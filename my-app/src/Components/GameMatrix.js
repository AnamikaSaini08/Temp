...
const GameMatrix = ({ row, col, carPosition, rotateCarClockWise, rotateCarAntiClockWise, image, endPosition, batteryPosition }) => {
  return Array.from({ length: row }, (_, rowIndex) =>
    Array.from({ length: col }, (_, colIndex) => {
      ...
      } else if (batteryPosition.some(coord => coord[0] === colIndex + 1 && coord[1] === rowIndex + 1)) {
        return (
          <div key={`${rowIndex}-${colIndex}`} className="border border-gray-600 w-12 h-12 xl:h-16 xl:w-16 2xl:h-20 2xl:w-20">
            <img
