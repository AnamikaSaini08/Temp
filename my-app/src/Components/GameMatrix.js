    ...
    return Array.from({ length: row }, (_, rowIndex) =>
      Array.from({ length: col }, (_, colIndex) => {
        if (colIndex === carPosition?.x && rowIndex === carPosition?.y) {
          return (
            <div key={`${rowIndex}-${colIndex}`} className="border border-blue-500 w-20 h-20 2xl:h-24 2xl:w-24">
              <img
          ...
          return (
            <div key={`${rowIndex}-${colIndex}`} className="border border-blue-500 bg-green-300 w-20 h-20 2xl:h-24 2xl:w-24"></div>
          );
        }
        else{
          return(
            <div key={`${rowIndex}-${colIndex}`} className="border border-blue-500 bg-gray-100 w-20 h-20 2xl:h-24 2xl:w-24"></div>
          )
        }
      })
    );
